...
class Tooltip extends React.Component {
    ...
    render() {
        const {children, label, multiline} = this.props,
            ...
            {x, y} = tooltipPos;
        const classNamesTooltip = classNames({[compStyles.show]: show}, compStyles.tooltip);

        const tooltip = <div ref={this.tooltipRef} style={{left: x, top: y}} className={classNamesTooltip}>{label}</div>;
