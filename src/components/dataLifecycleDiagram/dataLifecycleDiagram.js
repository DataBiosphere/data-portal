...
class DataLifecycleDiagram extends React.Component {
  ...
  dataLifecycleFlowPaths = [
    "processedDataRetrieveHCAData",
    "metaDataRetrieveHCAData",
    "rawDataRetrieveHCAData",
  ...
  getCardStyles = state => {
    ...
    let topPos =
      Number(stateButtonAttributes.y.value) +
      Number(stateButtonAttributes.height.value) +
      this.hoverPosition;

    // Calculate card left position
    let leftPos =
      state.name === "AnalysisPortals"
        ? this.hoverPosition
        : Number(stateButtonAttributes.x.value) + this.hoverPosition;

    return {
      borderRadius:
        state === this.dataLifecycleStates[0]
          ? "4px 0 4px 4px"
          : "0 4px 4px 4px",
      left: leftPos + "px",
      opacity: 1,
  ...
  getCardContainerStyles = state => {
    ...
    let cardContainerWidth =
      stateContainerWidth > this.cardWidth
        ? stateContainerWidth + this.hoverBounds
        : this.cardWidth + this.hoverBounds;
    let cardContainerHeight =
      stateContainerHeight + state.cardHeight + this.hoverBounds;

    // Calculate card container transform position - insert 'px' into transform value
    let cardContainerTranslate = stateContainerAttributes.transform.value
      .replace(",", "px,")
      .replace(")", "px)");

    let leftPos =
      state.name === "AnalysisPortals"
        ? stateContainerWidth - this.cardWidth - this.hoverPosition
        : -this.hoverPosition;
  ...
  removeFlowPathInteractionAndAttributes = () => {
    this.dataLifecycleFlowPaths.forEach(flowPath => {
      ...
      flowPathContainerEl.removeEventListener(
        "mouseenter",
        this.setFlowPathActiveClassAndAttributes(flowPathContainerEl)
      );
      flowPathContainerEl.removeEventListener(
        "mouseleave",
        this.clearFlowPathActiveClassAndAttributes(flowPathContainerEl)
      );
    });
  };
  ...
  removeStateInteractionAndAttributes = () => {
    this.dataLifecycleStates.forEach((state, i) => {
      if (state.text) {
        ...
        stateContainerEl.removeEventListener(
          "mouseenter",
          this.setStateActiveClassAndAttributes(stateContainerEl, i)
        );
        cardContainerEl.removeEventListener(
          "mouseleave",
          this.clearStateActiveClassAndAttributes(stateContainerEl, i)
        );
      }
    });
  };
  ...
  setStateActiveClassAndAttributes = (stateEl, i) => {
    return () => {
      ...
      stateEl.setAttribute(
        "filter",
        `url(#shadow${this.state.dataLifecycle.name})`
      );
    };
  };
  ...
  setFlowPathInteractionAndAttributes = () => {
    this.dataLifecycleFlowPaths.forEach(flowPath => {
      ...
      // Add interaction for flow path
      flowPathContainerEl.addEventListener(
        "mouseenter",
        this.setFlowPathActiveClassAndAttributes(flowPathContainerEl)
      );
      flowPathContainerEl.addEventListener(
  ...
  setStateInteractionAndAttributes = () => {
    this.dataLifecycleStates.forEach((state, i) => {
      if (state.text) {
        ...
        stateContainerEl.addEventListener(
          "mouseenter",
          this.setStateActiveClassAndAttributes(stateContainerEl, i)
        );
        cardContainerEl.addEventListener(
          "mouseleave",
          this.clearStateActiveClassAndAttributes(stateContainerEl, i)
        );
      }
  ...
  render() {
    let dataLifecycle = this.state.dataLifecycle
      ? this.state.dataLifecycle
      : "";
    return (
      <div
      ...
      >
        ...
        <div
          id="card"
          className={compStyles.dataLifecycleCard}
          style={
            dataLifecycle ? this.getCardContainerStyles(dataLifecycle) : null
          }
        >
          {dataLifecycle ? (
            <div id="card-contents" style={this.getCardStyles(dataLifecycle)}>
