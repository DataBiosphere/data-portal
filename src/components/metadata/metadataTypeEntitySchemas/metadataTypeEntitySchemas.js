...
function MetadataTypeEntitySchemas(props) {
    ...
    const typeCategory = MetadataService.findMetadataTypeEntityCategory(MetadataTypeEntityQuery(), category);
    const {schemas} = typeCategory;

    return (
        <div className={compStyles.schemas}>
            {schemas.map((schema, s) => <MetadataTypeEntitySchema key={s} schema={schema}/>)}
        </div>
    )
}
