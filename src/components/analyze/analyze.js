...
class Analyze extends React.Component {
    ...
    render() {
        return (
            <div>
                {this.props.linked.map((link, i) => <Link to={this.getMarkdownFileName(link)} className={compStyles.hcaAnalyzeList} key={i}>
                    <div>
                        <h5>{link.childMarkdownRemark.frontmatter.title}</h5>
                        {link.childMarkdownRemark.frontmatter.author ? <p className={fontStyles.xxs}>{link.childMarkdownRemark.frontmatter.author}</p> : null}
