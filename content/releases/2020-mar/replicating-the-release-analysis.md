...
This tutorial covers how to process an unnormalized count matrix (in loom format) for clustering, differential expression, and visualization using the Cumulus workflow in a Terra workspace. We will use the same [methods](data.humancellatlas.org/releases/2020-mar/methods) outlined for the DCP March 2020 Release. 
...
Notice the field requires a “File” input. Specify the Google bucket location for the unnormalized count matrix (loom) file. To do this, select the folder icon in the input_file attribute box. Choose the 2020-Mar-Landscape-Adult-Liver-10x.loom. 
