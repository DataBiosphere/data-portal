...
// App dependencies
const {buildSchemaJSONByPath, getMetadataEntities, getMetadataEntityCategories, getMetadataSchemas, getMetadataSchemaProperties,
    getSetOfMetadataEntities, getSchemaFilePathsByRelativePath} = require(path.resolve(__dirname, "../utils/metadata.service.js"));
const {getCategorySchemas, getEntityCategories, getFieldTypeUsedBy, getSchemaProperties} = require(path.resolve(__dirname, "../utils/metadata-field-extension.service.js"));
const {generateMetadataIndex} = require(path.resolve(__dirname, "../utils/metadata-index.service.js"));
...
exports.sourceNodes = async ({ actions, createNodeId, createContentDigest }) => {
    ...
    const schemaFilePathsByRelativePath = getSchemaFilePathsByRelativePath(schemaJSONByPath);
    ...
    const metadataEntityCategories = getMetadataEntityCategories(schemaFilePaths, setOfMetadataEntities);

    /* Build all metadata schema. */
    const metadataSchemas = getMetadataSchemas(schemaJSONByPath, schemaFilePathsByRelativePath);

    /* Build all metadata schema properties. */
    const metadataSchemaProperties = getMetadataSchemaProperties(schemaJSONByPath, schemaFilePathsByRelativePath);
    ...
    metadataEntityCategories.forEach(entityCategory => {
        ...
        const nodeMeta = {
            id: createNodeId(`${entityCategory.entity}${entityCategory.categoryName}`),
            parent: null,
            children: [],
            internal: {
...
exports.createSchemaCustomization = ({ actions }) => {
        ...
        extend(options, prevFieldConfig) {
            return {
                resolve(source, arg, context, info) {
                    const categories = context.nodeModel.getAllNodes({type: "MetadataEntityCategory"});
    ...
    createFieldExtension({
        ...
        extend(options, prevFieldConfig) {
            return {
                resolve(source, arg, context, info) {
                    const properties = context.nodeModel.getAllNodes({type: "MetadataSchemaProperty"});
    ...
    createFieldExtension({
        ...
        extend(options, prevFieldConfig) {
            return {
                resolve(source, arg, context, info) {
                    const schemas = context.nodeModel.getAllNodes({type: "MetadataSchema"});
    ...
    createFieldExtension({
        ...
        extend(options, prevFieldConfig) {
            return {
                resolve(source, arg, context, info) {
                    const metadataSchemaProperties = context.nodeModel.getAllNodes({type: "MetadataSchemaProperty"});
